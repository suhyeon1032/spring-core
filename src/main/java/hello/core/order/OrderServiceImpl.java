...
public class OrderServiceImpl implements OrderService { // implements OrderService => 의존하고 있다.
    ...
    private MemberRepository memberRepository; //memberRepository와 discountPolicy를 참조(의존)하고있다.
...
//    따라서 FixDiscountPolicy 를 RateDiscountPolicy 로 변경하는 순간 OrderServiceImpl 의 소스 코드도 함께 변경해야 한다
    private DiscountPolicy discountPolicy; // 2. 구체에 의존하지 않고 추상화인 인터페이스에만 의존한다. // 하지만 NullPointerException이 터진다.
